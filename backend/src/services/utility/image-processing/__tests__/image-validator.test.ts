...
const mockFileTypeFromBuffer = jest.mocked(fileType.fromBuffer);
...
describe('ImageValidator', () => {
  ...
  describe('validate', () => {
    ...
    it('should throw error for empty buffer', async () => {
      ...
      await expect(validator.validate(emptyBuffer, 'test.jpg')).rejects.toThrow(Error);
    ...
    it('should throw error for file exceeding max size', async () => {
      ...
      await expect(validator.validate(largeBuffer, 'large.jpg')).rejects.toThrow(Error);
    ...
    it('should throw error for invalid image format (text file)', async () => {
      ...
      await expect(validator.validate(textBuffer, 'test.txt')).rejects.toThrow(Error);
      await expect(validator.validate(textBuffer, 'test.txt')).rejects.toThrow('Unable to determine file type');
    });
    ...
    it('should throw error for invalid image format (PDF)', async () => {
      ...
      await expect(validator.validate(pdfBuffer, 'test.pdf')).rejects.toThrow(Error);
    ...
    it('should throw error for corrupted JPEG (incomplete magic bytes)', async () => {
      ...
      await expect(validator.validate(corruptedBuffer, 'corrupted.jpg')).rejects.toThrow(Error);
    });
    ...
    it('should throw error for very small invalid buffer', async () => {
      ...
      await expect(validator.validate(tinyBuffer, 'tiny.jpg')).rejects.toThrow(Error);
      await expect(validator.validate(tinyBuffer, 'tiny.jpg')).rejects.toThrow('Unable to determine file type');
    });
  ...
  describe('edge cases', () => {
    it('should throw error for buffer with null bytes', async () => {
      ...
      await expect(validator.validate(nullBuffer, 'null.jpg')).rejects.toThrow(Error);
    });
    ...
    it('should reject images that look like other formats', async () => {
      ...
      await expect(validator.validate(gifBuffer, 'test.gif')).rejects.toThrow(Error);

      // BMP magic bytes (not supported)
      const bmpBuffer = Buffer.from('BM');
      mockFileTypeFromBuffer.mockResolvedValue({ ext: 'bmp', mime: 'image/bmp' });
      await expect(validator.validate(bmpBuffer, 'test.bmp')).rejects.toThrow(Error);
    });
  });
});